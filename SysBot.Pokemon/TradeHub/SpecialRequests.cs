    ...
    public static class SpecialRequests
    {
        ...
        public static SpecialTradeType CheckItemRequest<T>(ref T pk, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail, string TrainerName, SaveFile sav) where T : PKM, new()
        {
            ...
            if (startingHeldItem > 0 && startingHeldItem < allitems.Length)
            {
                ...
                caller.Log("Item retenido: " + itemHeld);
            }
            else
            {
                caller.Log("El item retenido estaba fuera de los límites de la matriz o no se retuvo nada: " + startingHeldItem);
            }
            ...
            else if (pk.HeldItem >= 1231 && pk.HeldItem <= 1251) // lonely mint <> serious mint
            {
                ...
                {
                    detail.SendNotification(caller, "La solicitud de naturaleza no se encontró en la base de datos.");
                    sst = SpecialTradeType.FailReturn;
                    return sst;
                }
            ...
            else if (pk.Nickname.StartsWith("!"))
            {
                ...
                if (item < 0)
                {
                    detail.SendNotification(caller, "La solicitud de item no es válida. Verifique la ortografía y la gen.");
                    return sst;
                }
            ...
            else if (pk.Nickname.StartsWith("?") || pk.Nickname.StartsWith("？"))
            {
                ...
                if (item < 0)
                {
                    detail.SendNotification(caller, "La solicitud de pokeball no fue válida. Revisa la ortografía y la gen.");
                    return sst;
                }
        ...
        private static void LegalizeIfNotLegal<T>(ref T pkm, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail, string trainerName) where T : PKM, new()
        {
            ...
            if (!la.Valid)
            {
                detail.SendNotification(caller, "¡Esta solicitud no es legal! Intentando legalizar...");
                ...
                {
                    caller.Log("La legalización fracasó: " + e.Message); return;
                }
            }
            else
            {
