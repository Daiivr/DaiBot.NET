...
public sealed class RequireQueueRoleAttribute(string RoleName) : PreconditionAttribute
{
    ...
    public override Task<PreconditionResult> CheckPermissionsAsync(ICommandContext context, CommandInfo command, IServiceProvider services)
    {
        ...
        if (!canQueue)
            return Task.FromResult(PreconditionResult.FromError($"<a:no:1206485104424128593> {context.User.Mention} lo siento, actualmente no acepto solicitudes para entrar en la cola."));
