...
namespace SysBot.Pokemon.Discord
{
    public class SmogonTeam : ModuleBase<SocketCommandContext>
    {
        ...
        {
            var generatingMessage = await ReplyAsync($"<a:loading:1210133423050719283> {Context.User.Mention} Generando y enviando tu equipo Smogon. Espere por favor...");
            try
            {
                ...
                var format = gameVersion.ToLowerInvariant() switch
                {
                    ...
                    "swsh" => 8,
                    "sv" => 9,
                    _ => throw new Exception("<a:no:1206485104424128593> Versión del juego no válida."),
                };
                ...
                foreach (var set in team)
                {
                    ...
                    if (pk == null)
                    {
                        await ReplyAsync($"<a:warning:1206483664939126795> Error al generar {speciesName}.");
                        continue;
                    }
                ...
                using (var imageStream = new MemoryStream())
                {
                    ...
                    var embedBuilder = new EmbedBuilder()
                        ...
                        .WithAuthor(
                            ...
                            {
                                author
                                    .WithName($"{typeName} generado por {Context.User.Username}")
            ...
            {
                await ReplyAsync($"<a:warning:1206483664939126795> {Context.User.Mention} Error al generar el equipo Smogon: {ex.Message}");
            }
        }
        ...
        private static int GetRandomSpecies(string gameVersion, string type, HashSet<int> addedSpecies)
        {
            ...
            {
                throw new Exception("<a:warning:1206483664939126795> No hay más especies únicas disponibles.");
            }
