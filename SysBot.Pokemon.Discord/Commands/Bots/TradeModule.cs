...
public class TradeModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task TradeAsync([Summary("Trade Code")] int code, [Summary("Showdown Set")][Remainder] string content)
    {
        ...
        if (Info.IsUserInQueue(userID))
        {
            ...
            {
                Description = $"✘ {Context.User.Mention}, ya tienes una operación existente en la cola. Espere hasta que se procese.",
                Color = Color.Red,
    ...
    private async Task AddTradeToQueueAsync(int code, string trainerName, T pk, RequestSignificance sig, SocketUser usr, bool isBatchTrade = false, int batchTradeNumber = 1, int totalBatchTrades = 1, int formArgument = 0, bool isMysteryEgg = false)
    {
        ...
        if (!la.Valid)
        {
            ...
            string responseMessage = pk.IsEgg ? "⚠️ Conjunto de showdown no válido para este huevo. Por favor revisa tu información y vuelve a intentarlo.." :
