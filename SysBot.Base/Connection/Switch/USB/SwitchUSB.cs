...
public abstract class SwitchUSB : IConsoleConnection
{
    ...
    public void Connect()
    {
        SwDevice = TryFindUSB();
        if (SwDevice == null)
            throw new Exception("Dispositivo USB no encontrado.");
        if (SwDevice is not IUsbDevice usb)
            throw new Exception("El dispositivo utiliza un controlador USB Win. Utilice libusbK y cree un filtro.");
    ...
    protected byte[] PixelPeekUSB()
    {
        ...
        lock (_sync)
        {
            ...
            if (reader == null)
                throw new Exception("Dispositivo USB no encontrado o no conectado.");
            ...
            while (transfSize < size)
            {
                ...
                {
                    LogError($"Error al obtener la captura de pantalla: {UsbDevice.LastErrorString}");
                    Disconnect();
                    break;
                }
                transfSize += lenVal;
